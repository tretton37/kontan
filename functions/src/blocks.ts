...
import {getDay, getUpcomingWeekdayKeys, getWeek, weekdayKeyToDayStr} from "./utils";
...
export const homeScreen = ({presentUsers, plannedPresence, userId}
                               : { presentUsers: InboundDto[], plannedPresence: UpcomingPresenceDto[], userId: string }): View => {
  ...
  keys.forEach((key) => {
    ...
    if (plannedPresence.find((presence) => presence.users.some((user) => user.slackUserId === userId) && key === presence.key)) {
  ...
  return {
    ...
    blocks: [
      ...
      ...plannedPresence.map(({weekday, users}) => {
        const noOne = {
          "type": "section",
          "text": {
            "type": "mrkdwn",
